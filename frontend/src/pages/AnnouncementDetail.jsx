  ...
  if (!a) {
    return (
      <main className="max-w-3xl mx-auto px-6 py-24">
        <p className="text-[#7a1d2c] text-xl">Announcement not found.</p>
        <Link to="/announcements" className="inline-flex items-center gap-2 text-[#7a1d2c] underline mt-3">
  ...
  return (
    <main>
      ...
      <section className="max-w-3xl mx-auto px-6 lg:px-10 py-12">
        <div className="flex items-center gap-2 text-xs text-[#7a1d2c] font-sans-ui">
          <Megaphone size={14} /> Notice <span className="opacity-50">•</span>
          <Calendar size={12} /> {a.date}
        </div>
        <h2 className="text-3xl md:text-4xl text-[#7a1d2c] font-semibold mt-4" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
          {a.text}
        </h2>
      ...
      <section className="bg-[#f1e6d2]/60 py-14">
        <div className="max-w-5xl mx-auto px-6 lg:px-10">
          <h3 className="text-2xl text-[#7a1d2c] font-semibold mb-6" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Other Announcements</h3>
          <ul className="space-y-2">
            {others.map((o) => (
              <li key={o.id}>
                <Link to={`/announcement/${o.id}`} className="flex items-center justify-between bg-white p-3 border border-[#7a1d2c]/15 hover:border-[#7a1d2c]">
                  <span className="text-sm text-[#2a2a2a]">{o.text}</span>
                  <ChevronRight size={16} className="text-[#7a1d2c]" />
                </Link>
              </li>
            ))}
          </ul>
